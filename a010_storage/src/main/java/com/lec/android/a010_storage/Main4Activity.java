...
public class Main4Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String name =etName.getText().toString();
                if("".equals(name)){
                    tvResult.setText("delete fail-insert name");
                    return;
                }
                delete(name);
            }
    ...
    void update(String name, int age, String address){
       ...
       int cnt = db.update(tableName,values,"name=?",new String[]{name});
    ...
    //특정 name을 가진 record 삭제
    void delete (String name){
        int cnt = db.delete(tableName,"name=?",new String[]{name});
