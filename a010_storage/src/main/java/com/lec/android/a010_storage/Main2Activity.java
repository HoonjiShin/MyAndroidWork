...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //위험권한 획득
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && //Build.VERSION_CODES =>23
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    //저장 경호 가져오기
                    //Environment.getExternalStoragePublicDirectory(); API129 부터 deprecated 됨.
                    File path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
                    File f = new File(path, "external.txt"); //경로/파일명명
        ...
        btnRead.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!checkExternalStorage()) return; //외부 메모리 사용 여부 체크
    ...
    // 2-2 권한 획득 결과 받으면 실행되는 콜백
    @Override
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        switch (requestCode){
            case REQUEST_CODE:
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    Toast.makeText(this,"permission ok",Toast.LENGTH_LONG).show(); //this -> activity를 의미
                }else{
                    Toast.makeText(this,"permission denied",Toast.LENGTH_LONG).show();

                    //권한이 없으면 더이상 진행 불가하게 하든지 다른 조치를 취해야 한다.
                    //onDestory();
    ...
    protected boolean checkExternalStorage(){
        ...
        String state = Environment.getExternalStorageState();  // Environment.getExternalStorageState() 는 상태를 문자열로 뽑아낸다.
        String msg;

        //외부 메모리 상태
        if(Environment.MEDIA_MOUNTED.equals(state)){ // state가 장착되어있다 -> 읽기,쓰기 모두 가능
